...
impl MessageType {
    ...
    pub fn from_bits(bits: u8) -> Result<MessageType, &'static str> {
        match bits {
...
impl Message {
    ...
    pub fn get_head(&self) -> u8 {
        (self.msg_type.to_bits() << 6) | self.msg_length
    }
...
mod tests {
    ...
    #[test]
    fn test_message_type_to_bits() {
        assert_eq!(MessageType::ERROR.to_bits(), 0b00);
        assert_eq!(MessageType::REQUEST.to_bits(), 0b01);
        assert_eq!(MessageType::RESPONSE.to_bits(), 0b10);
    }

    #[test]
    fn test_message_type_from_bits() {
        assert!(matches!(
            MessageType::from_bits(0b00).unwrap(),
            MessageType::ERROR
        ));

        assert!(matches!(
            MessageType::from_bits(0b01).unwrap(),
            MessageType::REQUEST
        ));

        assert!(matches!(
            MessageType::from_bits(0b10).unwrap(),
            MessageType::RESPONSE
        ));

        assert!(MessageType::from_bits(0b11).is_err());
    }
    ...
    fn test_message_get_head() {
        let msg_type = MessageType::REQUEST;
        let msg_length = 0b0000_0100;
        let msg_payload = vec![0x01, 0x02];
        let payload_cpy = msg_payload.clone();
